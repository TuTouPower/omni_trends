...
> [!Warning]
> `pnpm dev` には互換性の問題があります。`pnpm build` + `node dist/output/server/index.mjs` を使用してください。

### データソースの追加

`shared/pre-sources.ts` でソースを定義し、`server/sources/` で実装します。詳しくは [CONTRIBUTING.md](CONTRIBUTING.md) をご覧ください。
...
[NewsNow](https://github.com/ourongxing/newsnow) by ourongxing をベースに開発。
